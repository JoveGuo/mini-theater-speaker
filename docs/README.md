# 项目文档索引

| 文档 | 内容 |
| --- | --- |
| [architecture.md](architecture.md) | 系统架构详细设计 |
| [audio_pipeline.md](audio_pipeline.md) | 音频管线权威定义 |
| [channel_mapping.md](channel_mapping.md) | 声道命名与映射规范 |
| [config_spec.md](config_spec.md) | 配置规范与 profile 格式 |
| [linux_os_selection.md](linux_os_selection.md) | Linux 系统选型（Ubuntu vs Debian/Buildroot/Yocto 等） |
| [hardware_rk3399.md](hardware_rk3399.md) | RK3399 硬件平台说明 |
| [hardware_bom.md](hardware_bom.md) | 硬件清单（BOM） |
| [linux_rk3399_setup.md](linux_rk3399_setup.md) | Linux 板卡接入指南 |
| [rk3399_sdk_bringup.md](rk3399_sdk_bringup.md) | RK3399 SDK 板级 Bring-up 状态与计划 |
| [measurement_method.md](measurement_method.md) | 测量与验收方法 |
| [signal_chain.md](signal_chain.md) | 信号链速览 |
| [roadmap.md](roadmap.md) | 开发路线图 |
| [development_guide.md](development_guide.md) | 开发与协作规范 |
